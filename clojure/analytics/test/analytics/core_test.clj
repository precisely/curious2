(ns analytics.core-test
	(:require [analytics.test-helpers :as th]
						[korma.db :as kd]
						[korma.core :as kc]
...
(facts "LIST data points of series."
	(fact "The table should be empty to start with."
		(let [user_id 1
					tag_id 1]
			(count (series-list user_id tag_id)) => 0)))
...
(facts "CREATE data point in series."
	(let [user_id 1
			 tag_id 2
			 amount 3.5
			 date (sql-time 1986 10 14)
			 description "tag 1"]
	 (do
		; Create two entries.
		(series-create user_id tag_id (dec amount) date "tag foo")
		(series-create user_id tag_id amount date description)

		(let [series (series-list user_id tag_id)
...
(facts "List the distinct user_ids."
	(doseq [x [1 2 3]]
		(let [user-id-1 1
					user-id-2 2
					tag_id (* 5 x)
					date (sql-time 1986 10 x)]
			(series-create user-id-1 (* 5 x) 42 date "tag 1")
			(series-create user-id-1 (* 5 x) 13 date "tag 1")
			(series-create user-id-2 (* 5 x) 7 date "tag 1")))

		(fact "There are 9 entries total."
			(series-count) => 9)
		(fact "There are 3 unique tag."
			(list-tag-ids 1) => [5 10 15])
...
(facts "Collect values into days and average them."
	(doseq [x [1 2 3]]
		(let [user-id-1 1
					...
					date (sql-time 1986 10 x)]
			(series-create user-id-1 tag_id amount				date descr)
			(series-create user-id-1 tag_id (* 2 amount)	date descr)
			(series-create user-id-2 tag_id (* 10 amount) date descr)))
...
(facts "Get daily average amounts from a series."
	(doseq [x [1 2 3]
		...
		(let [user-id 1
					...
					amount (* 10 x y)
					descr (str "tag " tag_id)
					date (sql-time 1986 10 y)]
			;(println (str user-id " " tag_id	" " amount " " date))
			(series-create user-id tag_id amount date descr)
			(series-create user-id tag_id (inc amount) date descr)))
	...
	(let [s1 {:user-id 1 :tag1-id 2 :tag2-id 3	:score 42 :overlap-n 5 :tag1-type "tag" :tag2-type "tag"}
		...
		(fact "After saving score1, we should be able to find it again."
			(do (score-save (s1 :user-id)
				 ...
				 (score-count)
					 => 1
				 (score-count (s1 :user-id))
					 => 1
				 (score-count (s1 :user-id) (s1 :tag1-id) (s1 :tag2-id))
					 => 1
				 (score-count 500)
					 => 0
				 (score-count (s1 :user-id) (s1 :tag1-id) (-> (s1 :tag2-id) inc))
					 => 0))
		 (fact "Saving should be idempotent."
			 (do
					 (score-save (s1 :user-id)
											 (s1 :tag1-id)
											 (s1 :tag2-id)
											 (s1 :score)
											 (s1 :overlap-n)
