...
class SearchQueryServiceGroup1Spec extends Specification {
	...
	void "test getUserSearchGroup1QueryString(#userId, #query, #followedUsersIds) returns '#expected'"() {
		...
		578		| "foo"	| [3]				|
		"(((publicName:(foo)) OR (publicBio:(foo)) OR (interestTagsString:(foo))) AND _type:user AND (_id:3) AND virtual:false)"
		
		4637	| "foo" | [6]				|
		"(((publicName:(foo)) OR (publicBio:(foo)) OR (interestTagsString:(foo))) AND _type:user AND (_id:6) AND virtual:false)"
	}	
}
