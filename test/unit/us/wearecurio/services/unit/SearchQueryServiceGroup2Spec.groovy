...
class SearchQueryServiceGroup2Spec extends Specification {
	...
	void "test getUserSearchGroup2QueryString(#userId, #query, #followedUsersIds) returns '#expected'"() {
		...
		expected
		34		| "foo"	| []				|
		"(((publicName:(foo)) OR (publicBio:(foo)) OR (interestTagsString:(foo)) OR (username:(foo)))  AND virtual:false AND _type:user AND NOT (_id:34))"
		
		578		| "foo"	| [3]				|
		"(((publicName:(foo)) OR (publicBio:(foo)) OR (interestTagsString:(foo)) OR (username:(foo)))  AND virtual:false AND _type:user AND NOT (_id:(3 OR 578)))"
		
		4637	| "foo" | [6]				|
		"(((publicName:(foo)) OR (publicBio:(foo)) OR (interestTagsString:(foo)) OR (username:(foo)))  AND virtual:false AND _type:user AND NOT (_id:(6 OR 4637)))"
	}	
}
