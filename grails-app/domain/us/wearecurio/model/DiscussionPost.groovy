...
>>>>>>> f5ca2d0 Undo origin/master merge
	...
	private static void unComment(Long authorUserId, Long discussionId, Long postId) {
		println "uncommenting..."
		println "isvalid..." + UserActivity.isValid(UserActivity.ActivityType.DELETE, UserActivity.ObjectType.DISCUSSION_POST)
		UserActivity.create(
			authorUserId,
	...
	public static void delete(DiscussionPost post) {
		...
		def postId = post.id
		def authorUserId = post.authorUserId
		post.delete(flush:true)
		unComment(null, discussion.id, post.id)
	}
