...
$(document).ready(function() {
	...
	$('#participants-list ul>li>ul').infiniteScroll({
		...
		onScrolledToBottom: function(e, $element) {
			this.pause();
			showMoreParticipants(${sprintInstance.hashid}, this);
		}
	});
...
function showMoreParticipants(sprintHashId, infiniteScroll) {
	if ((${sprintInstance.getParticipantsCount()} - offset) > 0) {
		queueJSON("Getting more participants", "/data/getSprintParticipantsData?id=" + sprintHashId
					...
					<div class="col-xs-10 sprint-content">
						...
						<div class="row">
							...
							<div class="right-content">
								...
								<i class="nav fa fa-chevron-right fa-4x pull-right" onclick="showMoreParticipants('${sprintInstance.hashid}')"></i>
							</div>
						</div>
					</div>
					<div class="col-xs-2">
						<g:if test="${sprintInstance.hasMember(user.id)}">
							<button id="leave-sprint" class="sprint-button" onclick="leaveSprint('${sprintInstance.hashid }')">Unfollow</button>
							<g:if test="${sprintInstance.hasStarted(user.id, new Date()) && !sprintInstance.hasEnded(user.id, new Date())}">
								<button id="stop-sprint" class="sprint-button prompted-action" 
										onclick="stopSprint('${sprintInstance.hashid }')">Stop</button>
								<button id="start-sprint" class="prompted-action sprint-button hidden"
										onclick="startSprint('${sprintInstance.hashid }')">Start</button>
							</g:if>
							<g:else>
						...
						<g:else>
							<a href="/home/joinSprint?sprintHashId=${sprintInstance.hashid }">
								<button id="join-sprint" class="sprint-button">Follow</button>
							</a>
						</g:else>
						<g:if test="${sprintInstance.hasAdmin(user.id)}">
							<button class="sprint-button" onclick="editSprint('${sprintInstance.hashid}')">Edit</button>
							<button class="sprint-button" onclick="deleteSprint('${sprintInstance.hashid}')">Delete</button>
						</g:if>
					</div>
				</div>
			</div>
